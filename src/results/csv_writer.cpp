...
namespace Results
{
  ...
  CSVWriter::CSVWriter(Subject& subject_, std::string const& path_)
      : subject(subject_), path(path_), stream()
  {
    // attach
  ...
  void CSVWriter::OpenFile()
  {
    ...
    if (!boost::filesystem::exists(fs_path, error_code))
      {
        BOOST_FILESYSTEM_THROW(boost::filesystem::filesystem_error("CSVWriter error", fs_path, error_code));
      }

    // still need to get absolute path if relative is supplied

    // open stream for writing
    stream.open(path + '/' + subject.results_meta.file_name + ".csv", std::fstream::out | std::fstream::trunc);
  }
