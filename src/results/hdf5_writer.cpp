...
namespace Results {
    ...
    , compression_level{ 3 }
  {
    ...
    } catch( H5::FileIException const& error ) {      // catch failure caused by the H5File operations
      error.printErrorStack();
      std::exit( EXIT_FAILURE );
    } catch( H5::DataSetIException const& error ) {   // catch failure caused by the DataSet operations
      error.printErrorStack();
      std::exit( EXIT_FAILURE );
    } catch( H5::DataSpaceIException const& error ) { // catch failure caused by the DataSpace operations
      error.printErrorStack();
      std::exit( EXIT_FAILURE );
    } catch( H5::AttributeIException const& error ) { // catch failure caused by the Attribute operations
      error.printErrorStack();
      std::exit( EXIT_FAILURE );
    }
  }
  ...
  static void AppendScalar( H5::DataSet* const dataset
    ...
    H5::DataSpace const mem_space( rank
                                   , dims
                                   , max_dims );
  ...
  void HDF5Writer::OpenFile() {
    ...
    file = std::make_unique<H5::H5File>( path + '/' + subject.results_meta.file_name + ".h5"
                                         , H5F_ACC_TRUNC );
  }
  ...
  void HDF5Writer::CreateScalarDataSets() {
    ...
    for( auto it_scalars = scalars.begin(); it_scalars != scalars.end(); ++it_scalars ) {
      ...
      std::unique_ptr<H5::DataSet> dataset =
        std::make_unique<H5::DataSet>
        ( file->createDataSet( group + '/' + set
                               , H5::PredType::NATIVE_DOUBLE
                               , dataspace
                               , prop_list ) );
