...
namespace YAML
{
  ...
  {
    static bool decode(Node const& node, Input::CaseSet& case_set)
    {
            ...
            if (node_key.as<std::string>() == "Case" && node_value.IsMap())
              {
                ...
                // pressure
                {
                  ...
                  cas.pressure.Set(node["Value"].as<double>(), node["Unit"].as<std::string>());
                }
                ...
                // temperature
                {
                  ...
                  assert(node.IsMap());
                  cas.temperature.Set(node["Value"].as<double>(), node["Unit"].as<std::string>());
                }
                ...
                // composition
                {
                  ...
                  if ("concentration" == type)
                    {
                      cas.composition.Set(node["Value"].as<Input::CompositionPairs>(), type, node["Unit"].as<std::string>());
                    }
                  else
                    {
                      cas.composition.Set(node["Value"].as<Input::CompositionPairs>(), node["Type"].as<std::string>());
                    }
                }
                ...
                // time
                {
                  ...
                  assert(node.IsMap());
                  cas.time.Set(node["Value"].as<double>(), node["Unit"].as<std::string>());
                }
...
namespace Input
{
  ...
  Parser::Parser(std::string const& config_file_)
      : config_file(config_file_), parameters()
  {
    Parse();
    Validate();
  }
  ...
  static void MechanismPathIsValid(std::string const& inp_path_str)
  {
    ...
    if (!boost::filesystem::exists(path, error_code))
      {
        BOOST_FILESYSTEM_THROW(boost::filesystem::filesystem_error("Input error", path, error_code));
      }
  }

  static void TemperatureIsValid(Cantera::ThermoPhase const& thermo, RealScalar const& temperature_meta)
  {
  ...
  static void CompositionIsValid(Cantera::ThermoPhase const& thermo, Composition const& composition_meta)
  {
    ...
    // check if species are unique
    {
      ...
      if (duplicate_species_found)
        {
          std::string str{};
          std::for_each(duplicate_species.begin(), duplicate_species.end(), [&str](auto const& species) { str += fmt::format("' '{}' '", species); });
