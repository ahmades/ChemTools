...
namespace SUNDIALS
{
  ...
  namespace CVODE
  {
    ...
    void IStrategy::SetClientData(void* const client_)
    {
      if (!client_)
        {
    ...
    boost::optional<int> IStrategy::SetInitStepSize(realtype const init_step)
    {
      if (init_step < 0.0)
        {
          throw std::invalid_argument("IStrategy::SetInitStepSize: "
    ...
    boost::optional<int> IStrategy::SetMinStepSize(realtype const min_step)
    {
      if (min_step < 0.0)
        {
          throw std::invalid_argument("IStrategy::SetMinStepSize: "
    ...
    boost::optional<int> IStrategy::SetMaxStepSize(realtype const max_step)
    {
      if (max_step < 0.0)
        {
          throw std::invalid_argument("IStrategy::SetMaxStepSize: "
    ...
    boost::optional<int> IStrategy::SetMaxNumSteps(long int const n_steps)
    {
      if (n_steps < 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxNumSteps: "
    ...
    void IStrategy::SetMaxOrder(int const max_order)
    {
      if (max_order < 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxOrder: "
    ...
    boost::optional<int> IStrategy::SetMaxWarnMessages(int const max_warn_msgs)
    {
      if (max_warn_msgs < 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxWarnMessages: "
    ...
    boost::optional<int> IStrategy::SetStabilityLimitDetection(bool const stab_lim_det_active)
    {
          ...
          if (Types::LinearMultisptepMethod::BDF == linear_multi_step_meth)
            {
              ...
              return CVodeSetStabLimDet(memory, static_cast<booleantype>(solver_ctrl.stab_lim_det_active));
            }
          else
            {
    ...
    boost::optional<int> IStrategy::SetMaxErrorTestFailures(int const max_err_test_fails)
    {
      if (max_err_test_fails <= 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxErrorTestFailures: "
    ...
    boost::optional<int> IStrategy::SetMaxNonlinearIterations(int const max_nonlin_iters)
    {
      if (max_nonlin_iters <= 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxNonlinearIterations: "
    ...
    boost::optional<int> IStrategy::SetMaxConvergenceFailures(int const max_conv_fails)
    {
      if (max_conv_fails <= 0)
        {
          throw std::invalid_argument("IStrategy::SetMaxConvergenceFailures: "
                                      "The maximum number of nonlinear iterations "
                                      "must be strictly positive.");
        }
                                                                        ...
                                                                        nonlin_conv_coef)
    {
      if (nonlin_conv_coef <= 0.0)
        {
          throw std::invalid_argument("IStrategy::SetNonlinConvergenceCoefficient: "
    ...
    int IStrategy::Initialise()
    {
      ...
      if (-1 == solver_ctrl.max_order)
        {
          solver_ctrl.max_order = (Types::LinearMultisptepMethod::BDF == linear_multi_step_meth)
                                      ? 5
                                      : 12;
        }
    ...
    int IStrategy::ReInitialise(realtype const time,
                                std::vector<realtype> const& state)
    {
      ...
      return CVodeReInit(memory, time, client->state);
    }
                                ...
                                realtype const* const state)
    {
      assert(is_initialised);
      realtype* const state_ptr = N_VGetArrayPointer(client->state);
      std::copy(state, state + client->n_states, state_ptr);
      return CVodeReInit(memory, time, client->state);
    }
    ...
    */
    ...
    realtype* IStrategy::Solution() const
    {
      return N_VGetArrayPointer(client->state);
    }

    realtype IStrategy::Solution(sunindextype const i) const
    {
      return N_VGetArrayPointer(client->state)[i];
    }

    int IStrategy::MainSolverStatistics(Types::MainSolverStatistics& stats) const
    {
      int ret_code;
      assert(memory);
    ...
    int IStrategy::LinearSolverStatistics(Types::LinearSolverStatistics& stats) const
    {
      int ret_code;
      assert(memory);
    ...
    std::string IStrategy::PrintSolverStatistics() const
    {
      std::string str;
      {
    ...
    int Dense::SetLinearSolver()
    {
      ...
      return CVodeSetLinearSolver(memory, linear_solver, matrix);
    }
    ...
    void Band::SetMatrixBandwidth(sunindextype const upper, sunindextype const lower)
    {
    ...
    int Band::SetLinearSolver()
    {
      ...
      matrix = SUNBandMatrix(client->n_states, bandwidth.upper, bandwidth.lower);
      // create the linear solver
    ...
    void Iterative::SetLinearSolverOptions(Types::Preconditioner const preconditioner, Types::GramSchmidt const gram_schmidt, int const n_krylov_basis_vectors, int const max_restarts)
    {
      options = {preconditioner, gram_schmidt, n_krylov_basis_vectors, max_restarts};
    }
    ...
    int Iterative::BindUserFunctions()
    {
      ...
      if (client->opt_udf_set.jacobian_times_vector)
        {
          CVLsJacTimesSetupFn jacobian_times_vector_setup_func = (client->opt_udf_set.jacobian_times_vector_setup)
