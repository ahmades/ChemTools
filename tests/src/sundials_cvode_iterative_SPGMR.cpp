...
public:
  ...
  CVDiurnalKry( Domain const& domain
    ...
    , m_prec_mat( m_domain.dim_x.npts
                  , std::vector<realtype**>( m_domain.dim_y.npts ) )
    , m_jac_mat( m_domain.dim_x.npts
                 , std::vector<realtype**>( m_domain.dim_y.npts ) )
    , m_pivot( m_domain.dim_x.npts
               , std::vector<sunindextype*>( m_domain.dim_y.npts ) )
  {
  ...
  int PreconditionerSetup( realtype const /*time*/
      ...
      for( int iy = 0; iy < m_domain.dim_y.npts; iy++ ) {
        ...
        for( int ix = 0; ix < m_domain.dim_x.npts; ix++ ) {
          ...
          j[0][0] = ( -m_phys_params.coef_q_1 * m_phys_params.coef_c_3
                      - m_phys_params.coef_q_2 * c2 ) + diag;
    ...
    for( int ix = 0; ix < m_domain.dim_x.npts; ix++ ) {
      for( int iy = 0; iy < m_domain.dim_y.npts; iy++ ) {
        ...
        sunindextype retval = denseGETRF( m_prec_mat[ix][iy]
                                          , m_nspecies
                                          , m_nspecies
                                          , m_pivot[ix][iy] );
...
TEST_CASE( "2-species diurnal kinetics advection-diffusion PDE "
           "system in 2 space dimensions can be solved", "[iterative solver]" ) {
